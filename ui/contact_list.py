...
class ContactListPanel(tk.Frame):
    ...
    def _create_contact_row(self, contact):
        ...
        tk.Label(avatar_frame, text=contact.get("first_name", " ")[:1].upper(),
                 font=FONT_MEDIUM, bg=group_color, fg=BG_PRIMARY).place(relx=0.5, rely=0.5, anchor="center")
        
        # اطلاعات
        info_frame = tk.Frame(row, bg=row.cget("bg"))
        info_frame.pack(side="left", fill="x", expand=True)
        
        # نام + ستاره اگر favorite بود
        name_text = f"{'⭐ ' if is_favorite else ''}{full_name}"
        tk.Label(info_frame, text=name_text, font=FONT_SMALL,
                 bg=row.cget("bg"), fg=TEXT_PRIMARY, anchor="w").pack(fill="x")
        
        tk.Label(info_frame, text=phone, font=FONT_TINY,
                 bg=row.cget("bg"), fg=TEXT_SECONDARY, anchor="w").pack(fill="x")
        
        # رویداد کلیک
        for widget in [row, avatar_frame, info_frame]:
